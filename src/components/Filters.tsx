...
export default function Filters() {
  ...
  const Group = ({
    ...
    <div className="border-b border-[var(--color-light-300)] py-4">
      <button
        className="flex w-full items-center justify-between text-body-medium text-[var(--color-dark-900)]"
        onClick={() => setExpanded((s) => ({ ...s, [k]: !s[k] }))}
        aria-expanded={expanded[k]}
        aria-controls={`${k}-section`}
      >
        <span>{title}</span>
        <span className="text-caption text-[var(--color-dark-700)]">{expanded[k] ? "−" : "+"}</span>
      </button>
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between md:hidden">
        <button
          className="rounded-md border border-[var(--color-light-300)] px-3 py-2 text-body-medium"
          onClick={() => setOpen(true)}
          aria-haspopup="dialog"
        >
          Filters
        </button>
        <button className="text-caption text-[var(--color-dark-700)] underline" onClick={clearAll}>
          Clear all
        </button>
      </div>

      <aside className="sticky top-20 hidden h-fit min-w-60 rounded-lg border border-[var(--color-light-300)] bg-[var(--color-light-100)] p-4 md:block">
        <div className="mb-2 flex items-center justify-between">
          <h3 className="text-body-medium text-[var(--color-dark-900)]">Filters</h3>
          <button className="text-caption text-[var(--color-dark-700)] underline" onClick={clearAll}>
            Clear all
          </button>
        </div>
              ...
              return (
                <li key={s}>
                  <label className="inline-flex items-center gap-2">
                    <input
                      type="checkbox"
                      className="h-4 w-4 accent-[var(--color-dark-900)]"
                      checked={checked}
        ...
        <Group title={`Price ${activeCounts.price ? `(${activeCounts.price})` : ""}`} k="price">
          <div className="flex space-x-2">
            <input
              type="number"
              placeholder="Min"
              className="w-1/2 rounded-md border border-[var(--color-light-300)] p-2 text-body"
              value={minPriceInput}
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true">
          <div
            className="absolute inset-0 bg-black/40 dark:bg-white/20"
            aria-hidden="true"
            onClick={() => setOpen(false)}
          />
          <div className="absolute inset-y-0 left-0 w-80 max-w-[80%] overflow-auto bg-[var(--color-light-100)] p-4 shadow-xl">
            <div className="mb-2 flex items-center justify-between">
